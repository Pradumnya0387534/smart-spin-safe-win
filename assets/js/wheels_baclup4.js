...
function initWheels() {
    ...
    if (!categoryWheel.canvas || !questionWheel.canvas) {
        ...
        console.error('[Wheels] categoryWheel:', categoryWheel.canvas);
        console.error('[Wheels] questionWheel:', questionWheel.canvas);
        return;
    }
    ...
    if (spinBtn) {
        ...
        console.error('[Wheels] Spin button not found');
    }
...
function drawWheel(wheel, segments) {
    ...
    for (let i = 0; i < segments; i++) {
        ...
        if (wheel === categoryWheel) {
            ...
            if (words.length > 2) {
                ctx.fillText(words.slice(0, 2).join(' '), WHEEL_RADIUS * 0.6, -8);
                ctx.fillText(words.slice(2).join(' '), WHEEL_RADIUS * 0.6, 8);
            } else if (words.length === 2) {
                ctx.fillText(words[0], WHEEL_RADIUS * 0.6, -5);
                ctx.fillText(words[1], WHEEL_RADIUS * 0.6, 5);
            } else {
 ...
 */
function spinWheels() {
    console.log('[Wheels] spinWheels() called');
    
    if (categoryWheel.isSpinning || questionWheel.isSpinning) {
        console.log('[Wheels] Already spinning, ignoring');
        return;
    }
    
    console.log('[Wheels] Starting spin...');
    ...
    console.log('[Wheels] Category final angle:', categoryFinalAngle);
    console.log('[Wheels] Question final angle:', questionFinalAngle);
...
function showSelection() {
    ...
    if (typeof loadQuestion === 'function') {
        ...
        console.error('[Wheels] loadQuestion function not found');
    }
}
...
function resetWheels() {
    ...
    // Show selection section, hide question section
