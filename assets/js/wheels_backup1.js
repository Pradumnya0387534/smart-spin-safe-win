...
// UPDATED: Question Numbers + Side Arrow + ABB Center + Fixed Category Selection
// ===================================
...
function drawCategoryWheel() {
    ...
    // Draw pointer (RIGHT SIDE - like your image)
    ctx.beginPath();
    ctx.moveTo(290, centerY);           // Right point
    ctx.lineTo(260, centerY - 15);      // Top left
    ctx.lineTo(260, centerY + 15);      // Bottom left
    ctx.closePath();
    ctx.fillStyle = '#FF000F';
    ctx.fill();
    ctx.strokeStyle = '#ffffff';
 ...
 * FIXED: Now correctly identifies the category at the arrow position (right side)
 * @returns {object}
 */
function getSelectedCategory() {
    ...
    // Since arrow is at 0 degrees (right side), we check which segment covers that angle
 ...
 * FIXED: Now correctly identifies the number at the arrow position (right side)
 * @returns {number}
 */
